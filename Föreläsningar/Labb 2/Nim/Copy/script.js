...
class Game
{
    ...
    throw()
    {
        ...
        for (let i = 0; i < this.stacks; i++) // lägger in 1 stick fram tills 21. this.stack = 21;
        {
    ...
    {
        let newPlayer = new Player(playername, /*myTurn*/ false);
        this.players.push(newPlayer);
        this.player[0].myturn = true;
    }
    get_player()
    {
        return this.players;
        ...
        this.players.forEach(p => 
            {
                if (p.myTurn)
                {
                    p.mySticks += number; // Addera hur mycket spelare dragit
                    this.throw_player_sticks(); // Addera hur mycket spelare har samlat
                }
            })
        this.change_player_turn(); // ändra den andra spelaren till true & nuvarande spelare till false
        this.player_is_active(); /// Det ska plaeras efter change_player_turn
    }
    endGame()
    {
        if (this.stack <= 0)
